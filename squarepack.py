...
def gen_image(g, d, cell=100):
    img = Image.new('RGBA', (cell*g.x, cell*g.y))
    sources = [os.path.join(d, x) for x in os.listdir(d)]
    print len(sources)
    for coord, spec in g.data.iteritems():
        src = sources.pop()
        print coord, spec[0], src
        src_img = Image.open(src).resize((cell*spec[0], cell*spec[0]))
